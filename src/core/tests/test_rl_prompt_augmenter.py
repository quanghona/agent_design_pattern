...
class TestRLPromptAugmenterIntegration:
    ...
    def test_train_with_simple_single_layer_policy(
        self, embedding_model, reward_model, temp_dir
    ):
        """Training with a simple single-layer policy model (not GPT2)."""
        augmenters = [
            IdentityPromptAugmenter(),
            SimplePromptAugmenter(format="{query} {data}"),
        ]
        ...
        aug = RLPromptAugmenter(
            ...
            policy_model_path=os.path.join(temp_dir, "policy.pt"),
            mode="train",
            policy_model_class=SimplePolicy,
        )
        result = aug.train(
        ...
        assert len(result["episode_rewards"]) == 3
        assert isinstance(result["best_reward"], float)
        # Verify the policy model is the simple single-layer type
        assert isinstance(aug.policy_model, SimplePolicy)

    def test_augment_with_simple_single_layer_policy(
        self, embedding_model, reward_model, temp_dir
    ):
        """Augment using a trained simple single-layer policy model."""
        augmenters = [IdentityPromptAugmenter()]
        env = PromptOptimizationEnv(
            initial_prompt="test prompt",
            augmenters=augmenters,
    ...
    def test_env_with_zero_augmenters_raises(self, embedding_model, reward_model):
        """Environment with 0 augmenters should raise ValueError."""
        with pytest.raises(ValueError, match="At least one augmenter must be provided"):
            PromptOptimizationEnv(
                initial_prompt="test",
                augmenters=[],
                embedding_model=embedding_model,
